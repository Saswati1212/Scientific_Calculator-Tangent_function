...
public class TanModel {
    ...
    public double tan(double x) throws Exception {
        ...
        System.out.println("Get the tan bro");

        try {
            // Check periodicity i.e. reduce x to the range 0<= x <= 180
            if (!(x >= 0.0 && x <= 180.0)) {
                System.out.println("in 0 to 180");
            ...
            throw new Exception("Problem occurred while calculating tan(x). Please try again. ");
        }
        return Double.NaN;
    }
}
